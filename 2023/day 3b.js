...
let reg_digits = /(\d+)/g, reg_symbols = /[\*]/g;

let symbols = schematic.map((v,i)=> {
  return Array.from(v.matchAll(reg_symbols))
  .map(v=>{ return {val: v[0], idx: v.index, row: i }})
}).filter(v=>v.length).reduce((p,c)=>{ return [...p, ...c]}, [])
symbols;

let numbers = schematic.map((v,i)=> {
  return Array.from(v.matchAll(reg_digits))
  .map(v=>{ return {val: parseInt(v[0]), idx_s: v.index-1, idx_e: v.index + v[0].length, row: i }})
}).filter(v=>v.length).reduce((p,c)=>{ return [...p, ...c]}, [])
numbers;

symbols.map(s=> numbers.filter(v=> s.row >= v.row-1 && s.row <= v.row+1 && s.idx >= v.idx_s && s.idx <= v.idx_e) )
.filter(v=>v.length == 2).map(v=>v[0].val * v[1].val ).reduce((p,c)=>p+c,0)
